...
def generateFacts(original_facts, storage_host):
    ...
    for host in original_facts.keys():
        ...
        if vm_facts_variant == 'storage':
            if (storage_host != storage_for_vm or ('create_storage' in config and not config[
                'create_storage'])) and (('pull_storage_from' not in config) or (
        ...
        if storage_type == 'blockdevice':
            ...
            if not any(d['name'] == fs_name for d in facts['zvols']):
                ...
                if vm_facts_variant == 'storage':
                    zvol['snapshots'] = {
                        'replicate_target': backup_prefix + storage_for_vm + '/vms/' + org + '/' + host}
                else:  # Backup variant
                    zvol['attributes']['readonly'] = 'on'

                # Create configuration for zfs-auto-snapshot script
                if vm_facts_variant == 'storage' and (
                        'local_snapshots' not in config and len(default_local_snapshots) > 0
            ...
            if 'pull_storage_from' in config and config['pull_storage_from'] == storage_host:
                facts['vm_facts_move_storages'].append(
                    {'source_host': storage_host, 'source_dataset': fs_name,
                     'target_host': storage_for_vm,
        ...
        # If the VM wants ZFS filesystems (the default), configure them and export them via NFS
            ...
            for fs in filesystems:
                ...
                if fs['name'] == 'root':
                    ...
                    if 'reservation' not in attributes:
                        attributes['reservation'] = facts[
                            'vm_facts_default_root_reservation'] if 'vm_facts_default_root_reservation' in facts else \
                            config['size']
                ...
                if fs_prefix + org + '/' + host + '-' + fs['name'] not in facts['zfs_filesystems']:
                    ...
                    if vm_facts_variant == 'storage':
                        zfs_fs['snapshots'] = {
                            'replicate_target': backup_prefix + storage_for_vm + '/vms/' + org + '/' + host + '-' + fs[
                                'name']
                        }
                    facts['zfs_filesystems'].append(zfs_fs)
                if 'pull_storage_from' in config and config['pull_storage_from'] == storage_host:
                    facts['vm_facts_move_storages'].append(
                        {'source_host': storage_host, 'source_dataset': fs_prefix + org + '/' + host + '-' + fs['name'],
                         'target_host': storage_for_vm,
